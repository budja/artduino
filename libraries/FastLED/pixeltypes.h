...
struct CRGB_lerp : public CRGB {
	uint8_t rTarget;
	uint8_t gTarget;
	uint8_t bTarget;
	uint8_t rStep, gStep, bStep;
	bool rStepSubtract, gStepSubtract, bStepSubtract;
	
	void lerpTo(CRGB &targetValue, uint8_t stepCount){
		if (stepCount == 0) {
			r = targetValue.r;
			g = targetValue.g;
			b = targetValue.b;
		...
		rTarget = targetValue.r;
		gTarget = targetValue.g;
		bTarget = targetValue.b;
		stepsRemaining = stepCount;
		uint8_t frac = 256 / stepCount;
		getStepSize(r, rTarget, rStepSubtract);
		rStep = scale8(getStepSize(r, rTarget, rStepSubtract), frac);
		gStep = scale8(getStepSize(g, gTarget, gStepSubtract), frac);
		bStep = scale8(getStepSize(b, gTarget, gStepSubtract), frac);
	}
	...
	void tick(){
		...
		if (stepsRemaining == 1){
			...
			r = rStepSubtract ? qsub8(r, rStep) : qadd8(r, rStep);
			g = gStepSubtract ? qsub8(g, gStep) : qadd8(g, gStep);
			b = gStepSubtract ? qsub8(b, bStep) : qadd8(b, bStep);
		}
		stepsRemaining--;
	}
	
	uint8_t getStepSize(uint8_t start, uint8_t end, bool &isSubtraction){
		short sStart = start;
		short sEnd = end;
		if ((sEnd - sStart) >= 0) {
			isSubtraction = false;
